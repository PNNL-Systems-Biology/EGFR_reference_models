    ...
    for i in range(egfLevels):
        ...
        try:
            ...
            tempSc = np.sqrt(np.sum((((eq["[RRp]"] + eq["[RRpE]"] + eq["[ERRpE]"] + 2 * (
                eq["[RpRp]"] + eq["[RpRpE]"] + eq["[ERpRpE]"])) - pegfr[i])/(pegfrStd[i]))**2) / tsNum)
        except:
            tempSc = 10 ** 12
...
error = 1.0e32
threshold = 0.1
trackfile = directory + "sims-test/trackingAll-" + tag + ".csv"
moniterfile = directory + "sims-test/monitorAll-" + tag + ".csv"
